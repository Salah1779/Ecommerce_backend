...
public class ProductService {
    ...
    public List<ProductDTO> filterProductsByCategory(Integer idCategory) {
        ...
        if(products.isEmpty()) {
            throw new ResourceNotFoundException("No products found in category with  " + idCategory);
        }
    ...
    public List<ProductDTO> filterByPriceRange(double minPrice, double maxPrice) {
        ...
        if(products.isEmpty()) {
            throw new ResourceNotFoundException("No product found in price range " + minPrice + " - " + maxPrice);
        }
        return products.stream().map(productMapper::toDTO).collect(Collectors.toList());
    ...
    public List<ProductDTO> filterProductsByCategoryAndPriceRange(Integer idCategory, double minPrice, double maxPrice) {
        ...
        if(products.isEmpty()) {
            throw new ResourceNotFoundException("No product found in category with id " + idCategory + " and price range " + minPrice + " - " + maxPrice);
        }
        return products.stream().map(productMapper::toDTO).collect(Collectors.toList());
    }
    ...
    public List<ProductDTO> searchProductsByLabel(String label) {
        ...
        if(products.isEmpty()) {
            throw new ResourceNotFoundException("No product found with label " + label);
        }
        return products.stream().map(productMapper::toDTO).collect(Collectors.toList());
    }
